...
<div id="messages">
	...
	<div class="row-fluid club-chat" data-message-id="{$message.id}">
		...
		<div class="span10" id="message{$message.id}">
			...
			<div class="club-show-link" data-comments-id="{$message.id}"><a data-action="comments">{if $message.comments_count}Показать комментарии ({$message.comments_count}){else}{if $user}Комментировать{/if}{/if}</a></div>	
		</div>
	</div>
	{/foreach}	
{if !$isAjax}</div>
